...
public class IdentityServerDialog extends BaseDialog {
    ...
    public void rebuild() {
        this.addCloseButton();
        this.cont.labelWrap(Settings.getText("identity.server-dialog.header")).fillX().center().get().setWrap(true);
        this.cont.row();
        for (User user : IdentitySettings.users.users()) {
            Table userTable = new Table();
            userTable.center();
            userTable.labelWrap(user.username()).fillX().center().get().setWrap(true);
            userTable.button(Icon.ok, Styles.emptyi, user::rejoinAs);
